<template>
  <div class="page-container">
    <h2 class="section-title">写作题库</h2>
    <div class="toolbar" role="group" aria-label="筛选">
      ...
      <el-select
        ...
        @change="onTaskChange"
      >
        <el-option label="Task 1" :value="1" />
        <el-option label="Task 2" :value="2" />
      </el-select>
      <el-select
        v-if="source === 'past'"
        v-model="year"
    ...
    <el-row v-else :gutter="16" class="cards-row" v-loading="loading && list.length">
      <el-col v-for="q in list" :key="q.id" :xs="24" :sm="12" :md="8">
        <QuestionCard :item="q">
          <template #head="{ item }">
            <el-tag size="small" :type="item.task === 1 ? 'warning' : 'danger'">Task {{ item.task }}</el-tag>
          </template>
          <template #extra-tags="{ item }">
            <el-tag v-if="item.wordLimit" size="small" effect="plain">≥{{ item.wordLimit }} 词</el-tag>
          </template>
          <template #foot="{ item }">
...
const { list, loading, error, page, pageSize, total, source, bandMax, year, filter, load, onFilterChange, onSizeChange, changeSource, changeBand, changeYear } =
  useQuestionList<WritingQuestion>('writing')

const taskFilter = ref<number | ''>('')
const bandOptions = [5.0, 5.5, 6.0, 6.5, 7.0, 7.5, 8.0, 8.5, 9.0]
const yearOptions = Array.from({ length: 25 }, (_, i) => 2001 + i)

function onTaskChange() {
  filter.value = taskFilter.value === '' ? {} : { task: taskFilter.value }
  onFilterChange()
}
</script>

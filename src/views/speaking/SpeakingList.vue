<template>
  <div class="page-container">
    <h2 class="section-title">口语题库</h2>
    <el-alert type="info" :closable="false" class="top-tip" title="当季口语话题 · Part 1/2/3 全覆盖，支持录音练习（二期将接入 AI 考官打分）" />
    ...
    <el-row v-else :gutter="16" class="cards-row" v-loading="loading && list.length">
      <el-col v-for="q in list" :key="q.id" :xs="24" :sm="12" :md="8">
        <QuestionCard :item="q">
          <template #head="{ item }">
            <el-tag size="small" type="danger">Part {{ item.part }}</el-tag>
          </template>
          <template #foot="{ item }">
            建议时长 {{ item.durationSec }}s<template v-if="item.year"> · {{ item.year }}</template>
          </template>
        </QuestionCard>
      </el-col>
    </el-row>
...
const { list, loading, error, page, pageSize, total, source, bandMax, year, filter, load, onFilterChange, onSizeChange, changeSource, changeBand, changeYear } =
  useQuestionList<SpeakingQuestion>('speaking')

const partFilter = ref<number | ''>('')
const bandOptions = [5.0, 5.5, 6.0, 6.5, 7.0, 7.5, 8.0, 8.5, 9.0]
const yearOptions = Array.from({ length: 25 }, (_, i) => 2001 + i)

function onPartChange() {
  filter.value = partFilter.value === '' ? {} : { part: partFilter.value }
  onFilterChange()
}
</script>
